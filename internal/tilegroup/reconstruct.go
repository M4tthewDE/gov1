...
func (t *TileGroup) reconstruct(plane int, x int, y int, txSz int, sh sequenceheader.SequenceHeader, uh uncompressedheader.UncompressedHeader, state *state.State) {
	...
	for i := 0; i < th; i++ {
		for j := 0; j < tw; j++ {
			var q int
			if i == 0 && j == 0 {
				q = t.getDcQuant(plane, sh, uh, state)
			} else {
				q = t.getAcQuant(plane, sh, uh, state)
...
// get_dc_quant( plane )
func (t *TileGroup) getDcQuant(plane int, sh sequenceheader.SequenceHeader, uh uncompressedheader.UncompressedHeader, state *state.State) int {
	if plane == 0 {
		return dcQ(uh.GetQIndex(0, t.SegmentId, state)+uh.DeltaQYDc, sh)
	} else if plane == 1 {
		return dcQ(uh.GetQIndex(0, t.SegmentId, state)+uh.DeltaQUDc, sh)
	} else {
		return dcQ(uh.GetQIndex(0, t.SegmentId, state)+uh.DeltaQVDc, sh)
	}
}

func dcQ(b int, sh sequenceheader.SequenceHeader) int {
	return Dc_Qlookup[(sh.ColorConfig.BitDepth-8)>>1][util.Clip3(0, 255, b)]
}

// get_ac_quant( plane )
func (t *TileGroup) getAcQuant(plane int, sh sequenceheader.SequenceHeader, uh uncompressedheader.UncompressedHeader, state *state.State) int {
	if plane == 0 {
		return acQ(uh.GetQIndex(0, t.SegmentId, state), sh)
	} else if plane == 1 {
		return acQ(uh.GetQIndex(0, t.SegmentId, state)+uh.DeltaQUAc, sh)
	} else {
		return acQ(uh.GetQIndex(0, t.SegmentId, state)+uh.DeltaQVAc, sh)
	}
}

func acQ(b int, sh sequenceheader.SequenceHeader) int {
	return Dc_Qlookup[(sh.ColorConfig.BitDepth-8)>>1][util.Clip3(0, 255, b)]
}
