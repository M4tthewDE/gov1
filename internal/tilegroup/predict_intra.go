...
func (t *TileGroup) predictIntra(plane int, x int, y int, haveLeft bool, haveAbove bool, haveAboveRight bool, haveBelowLeft bool, mode int, log2W int, log2H int, state *state.State, sh sequenceheader.SequenceHeader) {
	...
	for i := 0; i < w+h-1; i++ {
		if util.Int(haveAbove) == 0 && util.Int(haveLeft) == 1 {
			if x-1 == -1 {
				t.AboveRow[i] = state.CurrFrame[plane][y][len(state.CurrFrame[plane][y])-1]
			} else {
...
func (t *TileGroup) basicIntraPredictionProcess(w int, h int) [15][15]int {
	var pred [15][15]int
...
func (t *TileGroup) recursiveIntraPredictionProcess(w int, h int, sh sequenceheader.SequenceHeader) [15][15]int {
	w4 := w >> 2
	h2 := h >> 1
	var pred [15][15]int
	for i2 := 0; i2 <= h2-1; i2++ {
...
func (t *TileGroup) directionalIntraPredictionProcess(plane int, x int, y int, haveLeft int, haveAbove int, mode int, w int, h int, maxX int, maxY int, state *state.State, sh sequenceheader.SequenceHeader) [15][15]int {
	var pred [15][15]int
...
func (t *TileGroup) dcIntraPredictionProcess(haveLeft bool, haveAbove bool, log2W int, log2H int, w int, h int, sh sequenceheader.SequenceHeader) [15][15]int {
	var pred [15][15]int
...
func (t *TileGroup) smoothIntraPredictionProcess(mode int, log2W int, log2H int, w int, h int) [15][15]int {
	var pred [15][15]int
	if mode == SMOOTH_PRED {
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
